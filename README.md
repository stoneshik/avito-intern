...
Для запуска требуется файл `.env` где прописаны нужные переменные окружения, в качестве примера представлен файл `.env.example`, он находится в нужном месте (`./src/main/resources/.env.example`), для его использования достаточно убрать `.example` из названия.

При запуске под профилем `dev` бд наполняется тестовыми данными, для удобства тестов API через Postman я сохранил файл коллекции запросов: `"Avito Intern.postman_collection.json"`
...
Делаем миграцию базы данных (из корневой директории проекта):<br>
`"./mvnw" flyway:migrate -f "./pom.xml"` - запуск миграции<br>
`"./mvnw" flyway:clean -f "./pom.xml"` - очистка базы данных<br>
...
**Добавленный эндпоинт статистики:**<br>
GET: `/statistics/assignments/number-of-users` - получение количества назначенных на ревью пользователей
...
**GET: /users/getReview -> GET: /users/{user_id}/get-review**
...
Составленная даталогическая модель:<br>
![Даталогическая модель](Даталогическая_модель.png)

Тестовое покрытие:<br>
